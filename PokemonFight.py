...
anim_attack = AnimatedSprite(load_image("damage.png", -1), 3, 1, 190, 0)
all_sprites.add(anim_attack)
...
class Pokemon(pygame.sprite.Sprite):
    ...
    def anim_attack(self):
        if self.count == 2000000000000000:
            all_sprites.update()
            self.count = 0
        all_sprites.draw(game)
        self.count += 0.000000000000000000000000000000000000000000000000001
...
pokemons = [bulbasaur, charmander, squirtle, pikachu,
            jigglypuff, haunter, magikarp, rattata, pidgey]
player_pokemon = None
rival_pokemon = None
...
while game_status != 'quit':
    ...
    all_sprites.update()
    clock.tick(50)
    ms_from_last_tick = clock.tick(60)
    delta_t = ms_from_last_tick / 1000
            ...
            if game_status == 'select pokemon':
                for i in range(len(pokemons)):
                    if pokemons[i].get_rect().collidepoint(mouse_click):
                        ...
                        player_pokemon.level -= 2 * count_restart
                ...
                for i in range(len(player_pokemon.moves)):
                    ...
                    button = create_button(button_width, button_height,
                                           left, top, text_center_x, text_center_y, move.name.capitalize())
